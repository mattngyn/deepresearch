...
uv run vllm serve Qwen/Qwen2.5-7B-Instruct \
    ...
    --max-model-len 32768 --enable-lora --max-lora-rank 64 --max-cpu-loras 4 --enable-auto-tool-choice \
    --tool-call-parser hermes --disable-log-requests --dtype auto

torchrun --nproc-per-node 1 -m hud.rl.train --tasks deep_research_taskset_full_filtered_single.json --config deepresearch_rl_config.json --verbose
